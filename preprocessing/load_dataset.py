...
def analyze_image_properties(ds, output_dir: str = EDA_OUTPUT_DIR):
    """Analyze image size and format distribution."""
    os.makedirs(output_dir, exist_ok=True)
    ...
    aspects = [w/h if h > 0 else 0 for w, h in all_sizes]
    
    print(f"Image sizes: {len(set(all_sizes))} unique sizes")
    ...
    size_df.to_csv(os.path.join(output_dir, 'image_sizes.csv'), index=False)
    
    # Mode distribution
    mode_counts = Counter(all_modes)
    print(f"Image modes: {dict(mode_counts)}")
    pd.Series(mode_counts).to_csv(os.path.join(output_dir, 'image_modes.csv'))
    ...
    axes[2].hist(aspects, bins=30, edgecolor='black', alpha=0.7, color='green')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'image_size_distribution.png'), dpi=200, bbox_inches='tight')
    plt.close()


def analyze_split_statistics(ds, output_dir: str = EDA_OUTPUT_DIR):
    """Comprehensive statistics for each data split."""
    os.makedirs(output_dir, exist_ok=True)
    print("\nAnalyzing split statistics...")
    
    stats_data = []
    
    for split in ds.keys():
        data = ds[split]
        answers = [normalize_answer(a) for a in data['answer']]
        question_types = [derive_question_type(a) for a in data['answer']]
        
        # Count question types
        binary_count = sum(1 for qt in question_types if qt == 'binary')
        open_ended_count = sum(1 for qt in question_types if qt == 'open-ended')
        ...
        top_3_answers = answer_counter.most_common(3)
        ...
        stats_data.append({
            ...
            'binary_percentage': f"{binary_count/len(data)*100:.1f}%",
            'unique_answers': unique_answers,
            'top_answer': top_3_answers[0][0] if top_3_answers else 'N/A',
            'top_answer_count': top_3_answers[0][1] if top_3_answers else 0
        })
    
    stats_df = pd.DataFrame(stats_data)
    stats_df.to_csv(os.path.join(output_dir, 'split_statistics.csv'), index=False)
...
def create_comprehensive_report(ds, output_dir: str = EDA_OUTPUT_DIR):
    ...
    with open(report_path, 'w') as f:
        f.write("="*80 + "\n")
        f.write("VQA-RAD DATASET - EXPLORATORY DATA ANALYSIS REPORT\n")
        f.write("="*80 + "\n\n")
        ...
        f.write(f"Total samples across all splits: {total_samples}\n")
        for split in ds.keys():
            f.write(f"  {split}: {len(ds[split])} samples\n")
        f.write("\n")
        
        # Question statistics
        f.write("2. QUESTION STATISTICS\n")
        f.write("-" * 80 + "\n")
        all_questions = []
        for split in ds.keys():
        ...
        f.write(f"Average question length: {np.mean(question_lengths):.2f} words\n")
        f.write(f"Median question length: {np.median(question_lengths):.0f} words\n")
        f.write(f"Question length range: {min(question_lengths)} - {max(question_lengths)} words\n\n")
        
        # Answer statistics
        f.write("3. ANSWER STATISTICS\n")
        f.write("-" * 80 + "\n")
        all_answers = []
        for split in ds.keys():
            all_answers.extend([normalize_answer(a) for a in ds[split]['answer']])
        
        answer_counter = Counter(all_answers)
        f.write(f"Total unique answers: {len(answer_counter)}\n")
        f.write(f"Most common answers:\n")
        for answer, count in answer_counter.most_common(15):
            percentage = count / len(all_answers) * 100
            f.write(f"  '{answer}': {count} ({percentage:.1f}%)\n")
        f.write("\n")
        
        # Question type distribution
        f.write("4. QUESTION TYPE DISTRIBUTION\n")
        f.write("-" * 80 + "\n")
        all_question_types = [derive_question_type(a) for a in all_answers]
        type_counter = Counter(all_question_types)
        ...
        # Coverage analysis
        f.write("5. ANSWER COVERAGE\n")
        f.write("-" * 80 + "\n")
        cumulative = 0
        for k in [10, 20, 50, 100]:
            top_k = answer_counter.most_common(k)
            coverage = sum(count for _, count in top_k) / len(all_answers) * 100
            f.write(f"  Top-{k} answers cover: {coverage:.1f}% of dataset\n")
        
        f.write("\n" + "="*80 + "\n")
        f.write("Report generated successfully!\n")
        f.write("See 'eda_reports/' folder for detailed visualizations.\n")
        f.write("="*80 + "\n")
...
def visualize_dataset(ds, num_samples=5, output_dir: str = EDA_OUTPUT_DIR):
    ...
    for i, sample in enumerate(samples):
        ...
        # Truncate question and answer for display
        question_text = sample['question'][:60] + "..." if len(sample['question']) > 60 else sample['question']
    ...
    plt.close()
    print(f"✅ Sample visualizations saved to: {figure_path}")


def main():
    """Main function to load and explore the dataset with comprehensive EDA."""
    try:
        ...
        analyze_image_properties(ds, output_dir=EDA_OUTPUT_DIR)
        analyze_split_statistics(ds, output_dir=EDA_OUTPUT_DIR)
        visualize_dataset(ds, num_samples=8, output_dir=EDA_OUTPUT_DIR)
        create_comprehensive_report(ds, output_dir=EDA_OUTPUT_DIR)
        
        print("\n" + "="*80)
        print("✅ Dataset loaded and EDA completed successfully!")
        print(f"📁 All reports saved to: {EDA_OUTPUT_DIR}/")
        print("="*80)
        return ds
