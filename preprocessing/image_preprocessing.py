    ...
    def create_pytorch_transforms(self, is_training: bool = True) -> transforms.Compose:
        ...
        if is_training:
            transform_list = [
                ...
                # REMOVE RandomHorizontalFlip - laterality matters!
                transforms.RandomRotation(degrees=3),           # 5 → 3
                transforms.RandomAffine(
                    degrees=0,
                    translate=(0.05, 0.05),                     # 0.1 → 0.05
                    scale=(0.95, 1.05),                         # (0.9,1.1) → (0.95,1.05)
                    shear=3                                     # 8 → 3
                ),
                transforms.ColorJitter(
                    brightness=0.15,                            # 0.3 → 0.15
                    contrast=0.15,                              # 0.3 → 0.15
                    saturation=0.0,                             # REMOVE
                    hue=0.0                                     # REMOVE
                ),
                # REMOVE GaussianBlur completely
                transforms.ToTensor(),
